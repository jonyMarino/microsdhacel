...
Teclas::Teclas():teclas(0),teclaAnterior(0),teclaAnteriorTmp(0),rampa(0),contadorRampa(0){
  struct Method mChequeo = {
    staticChequear,
    this  
  };
  timerChequeo = new RlxMTimer(TIME_TIMER,mChequeo);
}
...
static void Teclas::staticChequear(void*self){
  ((Teclas*)self)->chequear();
}

void Teclas::chequear(){
  if(contadorTimePass)--contadorTimePass;
  
  if (teclaAnterior!=teclasPresionadas){

  	  for(int i=0;keys[i].nombre!=0;i++){
  	    if(keys[i].teclas==teclasPresionadas){
  	     tecla=keys[i].nombre;
  	     break;
  	    }
  	  }
  	  ...
  	  resetTimePass();
  	  teclaAnterior = teclasPresionadas;
  	  contadorRampa=0;
  	  rampa=KEY_RAMPA_INI;
  }
