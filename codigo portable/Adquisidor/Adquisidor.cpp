...
** ===================================================================
*/
Adquisidor::Adquisidor(const struct Array& _serializables,ConfiguracionAdquisidor& _configuracion):serializables(_serializables),configuracion(_configuracion):timer(_configuracion.getIntervalo()*1000,onTime,this){
  ...
  if(congiguracion.getEstrategia()==ERASENEXT)
    pfMemFullStrategy=Adq_EraseOnMemFull ; 
  else
    pfMemFullStrategy=Adq_StopOnMemFull ; 
  ...
  timer.stop();
	...
	{
	  estado = FULL;  
	  configuracion.setAdquirir(FALSE);		 // Poner la adquisición en no
	} 
	else if (configuracion.getAdquirir()==TRUE) {		 // la adquisición quedo en si al apagarse el equipo?								  
	  escribirPowerdown();                           // Header que indica que hubo un corte de Energia durante la adquisición
	  #ifdef DS1307
	  start();
	  #else
	  estado = HULT;
...
** ===================================================================
*/
void Adquisidor::onTime(void * _self){
  Adquisidor * self = (Adquisidor *)_self;
  
  if (self->estado==SI){
    self->grabando = TRUE;
    self->grabarParametros();
    self->grabando = FALSE;
  }
