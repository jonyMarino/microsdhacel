...
void EscrituraDemorada_defConstructor(void * _self,va_list *args){
  struct EscrituraDemorada* self = (struct EscrituraDemorada*) _self;
  self->nodo = va_arg(*args,struct NodoICModBus*);
...
void com_Init(void){
  Comu_Init();
  comProps= (struct Array *) _new(&Array,10);
}
/*
** ===================================================================
**    Function    : com_Init 
...
struct NodoICModBus * com_Add(const struct ClassPropiedad ** prop,void * Obj,word dir_ini,word dir_end){
  struct Array * getters = (struct Array *)_new(&Array,dir_end - dir_ini + 1);
  ...
  p_tmp= (struct NodoICModBus *)_new(&NodoICModBus,getters,Obj,dir_ini,dir_end);
...
word ModBus_getAddressData(word address){
  ...
  byte count = Array_count(comProps);
    ...
    for(i=0;i<count;i++){	
      void * nodo = Array_get(comProps,i);															
      if(NodoICModBus_contieneDireccion(nodo,(void*)address))
        return NodoICModBus_getDato(nodo,(void*)address);
    }
...
byte ModBus_writeAddress(word address,int dato){
  ...
  for(i=0;i<count;i++){	
      void * nodo = Array_get(comProps,i);																	
      if(NodoICModBus_contieneDireccion(nodo,(void*)address)){
       // if(instanceOf(getter,(const struct Class * const)&PropiedadGenerica))     cambiar: poner aqui
        if(pEscrituraDemorada)
