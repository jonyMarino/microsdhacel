...
#endif
byte IndiceFlash[PARAMETROS/8+1]; /* Indice de parametros modificados*/
////////////////////////////////////////////////
...
/* Write your code here */   
  ...
  PWM45_Init();		 // Inicializacion PWM    
 	...
 	for(i=0;i<CANTIDAD_CANALES;i++)
 		A_Sensor(i,PRom[R_Sensor+i],PRom[R_Decimales+i]);									//Limites para el sensor y dec y el PTSL
...
////////////////// ELECCION DE SET POINTS ////////////////
     for (i=0;i<CANTIDAD_SAL_CONTROL;i++){
        ...
        #else
		    else if(PRom[R_Programa+i]!=-1) SetPoint[i] = SetPointPrograma[i] ;
		    else SetPoint[i] = PRom[R_SetPoint+i]; 
        #endif
     }
//////////////////////////////////////////////////////////
...
/////////////////// Calculos para los canales leidos//////////  
        ...
        ValCont[i] =ValFinal[i]+integ[i]+deriv[i];
...
////////////////CONTROL 1//////////////////////////////////
        if (PRom[R_Tcontrol1] == _pid )		//Estoy en PID y no en autosintonia ???
...
//      for(i=0;i<)  		
        ...
        PWM_SetRatio16(duty_cont_ch[0],0);
	      ...
	      switch(St_Step[0]){
        case 0: if (ValFinal[0]> SetPoint[0]){  //espero a estar arriba
            St_Step[0]++;
            St_Counter[0]=0;
            main_text[3]='1';
            }
            break;
        case 1: if (ValFinal[0]< SetPoint[0]){ //espero a estar abajo 
                St_Step[0]++;
      			    main_text[3]='2';
      			    prev_val= (int)ValFinal[0];
      			    St_Counter[0]=0;
                }
      			   break;
        case 2: if (prev_val<ValFinal[0]){
              St_Step[0]++; //encontre el minimo??
              main_text[3]='3';
              St_Counter[0]=0;
              prev_val=ValFinal[0];//actualizar valor
              } else prev_val=ValFinal[0]; //no,actualizar valor 
      				break;
	      case 3: if (ValFinal[0]> SetPoint[0]){	      //espero a estar arriba
              St_Step[0]++;
	            main_text[3]='4';
	            }
	            break;
        case 4: if (prev_val>ValFinal[0]){
              St_Step[0]++; //encontre el maximo??
              main_text[3]='5';
              St_val_max=ValFinal[0];
              prev_val=ValFinal[0];//actualizar valor
              } else prev_val=ValFinal[0]; //no,actualizar valor 
      				break;
	      case 5: if (ValFinal[0]< SetPoint[0]){       //espero a estar abajo 
              St_Step[0]++;
	            main_text[3]='6';
	            }
	            break;
        case 6: if (prev_val<ValFinal[0]){
               //encontre el minimo?? Si->termine
              Cpu_DisableInt();
							  if(St_Counter[0]<St_T_MIN){
	              EscribirParametro(R_Stn,St_Err);
	              }else{
	              EscribirParametro(R_ABA1,(int)((St_val_max-ValFinal[0])*3)/2);
