...
#ifdef _PRINTER
 const struct MIPConf confMI ={
  10,
  FALSE
};

 const struct EPM203Conf epm203Conf={
  0,1
};
#endif

#ifdef RPM
  const SensorRpmConf config=DEFAULT_SENSOR_RPM_CONF;
#endif
...
//PresentaValor val(&SetPoint[0],"setpoint");
SensorAdaptador sensorAdaptado0(0,NULL);
EPM203Manejador os((struct EPM203Conf*)&epm203Conf);
ManejadorImpresionPersistente mi(os,(struct MIPConf*)&confMI);
#endif
#ifdef RPM
struct Capturador * cap;
struct SensorRpm sensorRpm;
ThreadAttachable mainThread;
...
{ 
 ...
 // newAlloced(&os,&EPM203Manejador,&epm203Conf);
  ...
  //mi.add(&val);
  mi.add(&sensorAdaptado0);
  
 // mi.add(&sensorAdaptado1);
  #else
  AS1_Init();
  #endif
  ...
  cap=(struct Capturador*)GET_INSTANCE();
  ...
  void * adjuntador= mainThread.getAdjuntador();
  newAlloced(&sensorRpm,&_SensorRpm,adjuntador,1000,cap,&config,"SEn rPM");
 _delete(adjuntador);
