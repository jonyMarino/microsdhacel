...
const struct TControlClass TableControlPID={
  &Class,
  "",
  &Object,
...
** ===================================================================
*/
void PID_Constructor(void * self,void * conf,void * sensor,void* salida){
...
*/
void PID_DefConstructor(void * self,va_list * args){
  PID_Constructor(self,va_arg(*args,void*),va_arg(*args,void*),va_arg(*args,void*));  
}
...
void PID_AddOnTSalChange(void * _self,void(*method)(void*,void*),void * Obj){
  struct ControlPID * _cont = _self;  
  
  if(!_cont->OnTipoSalidaChange)
    _cont->OnTipoSalidaChange=_new(&MethodContainer); 
  
  {
  struct Method * methodTmp = _new(&Method,method,Obj);    
  MethodContainer_add(_cont->OnTipoSalidaChange,methodTmp);
  }
  (*method)(Obj,_self);
}
/*
...
** ===================================================================
*/
const struct ISalida* PID_getSal(void * self){
  struct ControlPID * _cont = self;
  return _cont->salida;
}
...
void PID_setModSal(void * self,T_SalidaControl val){
  ...
  if(_cont->OnTipoSalidaChange){
    ...
    MethodContainer_iterator(_cont->OnTipoSalidaChange,&it); 
    
    while(MethodContainerIterator_hasNext(&it)){    
      const struct Method* mo= MethodContainerIterator_next(&it);
...
int PID_getConfSP(void * self){
  struct ControlPID * _cont = self;
  return  _MANEJADOR_MEMORIA_GET_WORD(pFlash,&_cont->_conf->iSP);
}
...
byte PID_setConfSP(void * self,int val){
  struct ControlPID * _cont = self;
  byte err=_MANEJADOR_MEMORIA_SET_WORD(pFlash,& _cont->_conf->iSP,val);
  return err;
}
...
byte set_LimInf_SP(void * self,int val){
  struct ControlPID * _cont = self;
  byte err=_MANEJADOR_MEMORIA_SET_WORD(pFlash,& _cont->_conf->iLimInfSP,val);
  if(!err)
...
byte set_LimSup_SP(void * self,int val){
  struct ControlPID * _cont = self;
  byte err=_MANEJADOR_MEMORIA_SET_WORD(pFlash,& _cont->_conf->iLimSupSP,val); 
  if(!err)
    if( _cont->_conf->iSP>val)
      err= Cntrl_setSP(_cont,val);
  return err;   
...
byte set_Reset(void * self,int val){
  struct ControlPID * _cont = self;
  return _MANEJADOR_MEMORIA_SET_WORD(pFlash,& _cont->_conf->iReset,val);
}
...
byte set_TipoControl(void * self,int tc){
  struct ControlPID * _cont = self;
  byte err=_MANEJADOR_MEMORIA_SET_BYTE(pFlash,& _cont->_conf->eTipoControl,tc); 
  if(!err)   
...
byte set_SalControl(void * self,int val){
  struct ControlPID * _cont = self;
  byte err= _MANEJADOR_MEMORIA_SET_BYTE(pFlash,& _cont->_conf->eSalidaControl,val);
  if(!err){    
...
byte set_H_Control(void * self,int val){
  struct ControlPID * _cont = self;
  byte err;  
  err=_MANEJADOR_MEMORIA_SET_WORD(pFlash,& _cont->_conf->iHisteresis,abs(val));
...
byte set_integral(void * self,int val){
  struct ControlPID * _cont = self;
  byte err= _MANEJADOR_MEMORIA_SET_WORD(pFlash,&_cont->_conf->iIntegral,val);
  if(!err)
    _cont->suminteg=0;
  return err;
}
...
byte set_derivada(void * self,int val){
  struct ControlPID * _cont = self;
  byte err= _MANEJADOR_MEMORIA_SET_WORD(pFlash,&_cont->_conf->iDerivada,val);
  return err;
}
...
byte set_PotenciaInf(void * self,int val){
  struct ControlPID * _cont = self;
  return _MANEJADOR_MEMORIA_SET_WORD(pFlash,&_cont->_conf->iPotenciaInf,val);
}
...
byte set_PotenciaSup(void * self,int val){
  struct ControlPID * _cont = self;
  return _MANEJADOR_MEMORIA_SET_WORD(pFlash,&_cont->_conf->iPotenciaSup,val);
}
...
byte set_PotenciaManual(void * self,int val){
  struct ControlPID * _cont = self;
  return _MANEJADOR_MEMORIA_SET_WORD(pFlash,&_cont->_conf->iPotenciaManual,val);
}
