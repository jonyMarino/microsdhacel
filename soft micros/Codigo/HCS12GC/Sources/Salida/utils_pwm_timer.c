...
void PWM_init(struct PWMTimer * pwm,byte pin){
  pwms[pin]=pwm;
  TIOS |= 1<<pin;
  TTOV &= ~(1<<pin);
}
...
void pwm_timer_set_periodo(struct PWMTimer * pwm,ulong periodo){
...
/* Setea la variable del duty de forma segura mientras se esta 
en modo proporcional*/
void pwm_timer_set_duty(struct PWMTimer * pwm,ulong duty){
  uchar salida = pwm->PinNum;
  dis_timer_interrupt(salida);
  pwm->ticksDuty =  duty;
  en_timer_interrupt(salida);
...
** ===================================================================
*/
byte setPWM_period(struct PWMTimer * pwm,byte Mode)
{
...
byte PWM_SetRatio16(struct PWMTimer * pwm,word Ratio)		/*Ratio es un valor de 0 a 1000*/
{
  ulong duty;
...
** ===================================================================
*/
static dword Conf_Toggle(struct PWMTimer * pwm,bool * next_Value)
{
