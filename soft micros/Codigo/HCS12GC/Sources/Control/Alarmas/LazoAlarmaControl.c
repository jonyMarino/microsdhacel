...
void LazoAlarmaControl_constructor(void * _self, 
  ...
  struct LazoAlarmaControl * self = _self;
...
/**/
void LazoAlarmaControl_defConstructor(void * _self,va_list * args){
  LazoAlarmaControl_constructor(_self,va_arg(*args,void*),va_arg(*args,void*),va_arg(*args,void*));  
}
...
static void factoryControl(struct LazoAlarmaControl * self, TControlAlarma control){
  ...
  switch(control){             
    case CONTROL_PROPORCIONAL:               
        newAlloced(&self->control,&CntrProporcionalInvClass, _ConfLazoCntrlAlSen_getConfControl(self->configuracion),sensor);        
      break;
    case CONTROL_RELATIVO: 
        newAlloced(&self->control,&CntrRelativoClass, _ConfLazoCntrlAlSen_getConfControl(self->configuracion),sensor,self->lazoControl);
      break;
    case CONTROL_BANDA: 
    default:  //error 
        newAlloced(&self->control,&CntrBandaClass, _ConfLazoCntrlAlSen_getConfControl(self->configuracion),sensor,self->lazoControl);
      break;      
  } 
