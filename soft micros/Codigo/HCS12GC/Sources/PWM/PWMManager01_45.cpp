...
PWMManager01_45::PWMHard45* PWMManager01_45::pwmHard45=NULL; 
 ...
 void PWMManager01_45::PWMHard01::PWMHard01(struct ManejadorMemoria &_manejadorMemoria,const TConfPWM &_conf): PWMHard(_manejadorMemoria,_conf){
  ...
  #ifdef __MODRR
    setReg8Bits(MODRR, 2);//2  //cablear al puerto T
    setReg8Bits(DDRT, 2);//2 
  #endif
...
void PWMManager01_45::PWMHard01::setPotenciaGuardada() {
 ...
 if(PWME_PWME1){		 /* PWM?*/    
    ...
    if(potencia==0)
      #ifdef __MODRR
      clrReg8Bits(PTT, 2);//2              /* PTT3=0 */
      #else
      clrReg8Bits(PTP, 2);//2 
      #endif
    else
      #ifdef __MODRR
      setReg8Bits(PTT, 2);//2            /* PTT3=1 */
      #else
      setReg8Bits(PTP, 2);//2
      #endif 
  }
...
/*PWMManager01_45::PWMManager01_45 (ManejadorMemoria &_manejadorMemoria,const TConfPWM &_conf01,const TConfPWM &_conf45){
   ...
   if( !pwmHard01)
    pwmHard01 = new PWMHard01(_manejadorMemoria,_conf01); 
   
   if( !pwmHard45)
    pwmHard45 = new PWMHard45(_manejadorMemoria,_conf45); 
   
    // poolPwm01=&pwmHard01;
     //poolPwm45=pwmHard45;
     
   // PWMHard01 pwm01(_manejadorMemoria,_conf01);
