...
bool PromBkp_grabacionOBorradoEnEspera(void * self){
  ...
  return ( _p->aGrabar || !LinkedList_Vacia(&_p->listaDireccionesABorrar)); 
}
/*
** ===================================================================
...
byte PromBkp_borrarPagina(void * self,void * Addr){
  ...
  if(Addr==NULL)
    return 1;
  return LinkedList_add(&_p->listaDireccionesABorrar,Addr);
}
...
byte PromBkp_grabarOBorrar(void * self){
  struct PromBkp * _p = self;
  void* direccion;
  direccion=LinkedList_Eliminar_Primero(&_p->listaDireccionesABorrar);
