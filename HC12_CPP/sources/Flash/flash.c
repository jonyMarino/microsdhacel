...
/* MODULE IFsh10. */
#include "Cpu.h"
#include "Memoria/Prom/Flash/Hard/flash.h"
...
** ===================================================================
*/
byte Flash_writeArray(void * Address,word From, word To, word* Array)
{
  word Addr=(word)Address;
  byte err;
  word i;
...
byte Flash_grabarFlash(void * PageAddr, word* Array)
{
  byte err;
   ...
   if (FSTAT_CCIF == 0)                /* Is previous command complete ? */
    return ERR_BUSY;                   /* If yes then error */
   err=Flash_eraseSector(PageAddr);  /* Erase sector */
    if(err)
      return(err);                     /* Return error code if previous operation finished not correctly */
   err = Flash_writeArray(PageAddr,0,PAGE_SIZE,Array); /* Restore sector */
...
bool Flash_isInPage(void * address,void* pagina){
  return Flash_getPage(address)==pagina;
}
