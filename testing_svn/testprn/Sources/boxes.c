...
byte A_Decimales(int decimales,byte chan){			  
 ...
 decimal_chan = SENSOR_Decimales(sensor);
      ...
      #endif  
        Lim_Inf_ch[chan]= SENSOR_Minimo(sensor)/div_dec[decimal_chan-decimales];
        Lim_Sup_ch[chan]= SENSOR_Maximo(sensor)/div_dec[decimal_chan-decimales];
      }else {
