...
//		    WriteWord((word)&PRom[R_SetPoint],1245);		 
  ...
  // Aqui veo el Power Sense para salvar datos 
    ...
    {		 
    pwsn++;									                               // inc counter
      if (pwsn==35){                                       // is counter = 30?				
      Cpu_DisableInt();
      //NICOLAS
      #ifndef programador
       flagFinMeseta=2;
      
      EscribirWord(ADR_CORTE_ENERGIA,flagFinMeseta);
      #endif
      #ifdef jony_05_07
      O2_PutVal(0);
			O4_PutVal(0);
...
if(vpout == OPEN){
  PWM_OnToggle(TRUE,outc1);
  PWM_OnToggle(FALSE,outa2);
}
else if(vpout == CLOSE){
  PWM_OnToggle(FALSE,outc1);
  PWM_OnToggle(TRUE,outa2);
}
else{
  PWM_OnToggle(FALSE,outa2);
  PWM_OnToggle(FALSE,outc1);
...
//////////////////////////////////////////
    ...
    Display1_PutVal(0);		 /* Borro el display */
      ...
      if ((PtrBox.Num==&Principal1.DirProc))FstTime = TRUE; //Modificar!!!!!
