...
void main(void)
{
  ...
  Cuenta = SavedCuenta;
  ValProc = Cuenta * FactorEscala[0] / 10000; 
   ...
   // WDog1_Clear();
    ...
    (*PtrTmp)();            // Funcion para el box correspondiente; llama a Num Handler, TextHandler, etc.	
		...
		if(!(ContFlags&STOP_MASK)){
		  Cpu_DisableInt();
		  ValProc = Cuenta * ((float)FactorEscala[0] / 10000); 
		  Cpu_EnableInt();
		}
		/*Control*/
		if((ValProc>=SetPoint[0]) && (!(ContFlags&SP1_MASK))){
		  Rele1_PutVal(led[0]=(ModoSetPoint[0]==EXCESO)?1:0);
		  CntRele[0] = TiempoRele[0]*100; 
		  ContFlags|=SP1_MASK;
		}
		if(ValProc>=SetPoint[1] && !(ContFlags&SP2_MASK)){
      ...
      Totalizador[0] = (PreTotalizador + Cuenta)* ((float)FactorEscala[1] / 10000);
		...
		if(ModoRele[1]==AUTOMATICO && !CntRele[1]){
		  Rele2_PutVal(led[1]=(ModoSetPoint[1]==EXCESO)?0:1);		  
		  ContFlags=0;
		}
