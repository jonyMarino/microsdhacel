...
void _EntryPoint(void)
{
  ...
  *(byte*)INITRG_ADR = 0;              /* Set the register map position */
  ...
  setReg8(INITRM, 25);                 /* Set the RAM map position */ 
  /* MISC: ??=0,??=0,??=0,??=0,EXSTR1=1,EXSTR0=1,ROMHM=0,ROMON=1 */
  setReg8(MISC, 13);                    
  /* PEAR: NOACCE=0,??=0,PIPOE=0,NECLK=0,LSTRE=0,RDWE=0,??=0,??=0 */
  setReg8(PEAR, 0);                     
  /*  System clock initialization */
  /* CLKSEL: PLLSEL=0,PSTP=0,SYSWAI=0,ROAWAI=0,PLLWAI=0,CWAI=0,RTIWAI=0,COPWAI=1 */
  setReg8(CLKSEL, 1);                  /* Select clock source from XTAL and set bits in CLKSEL reg. */ 
